...
public class KeyBoardUtils {
    ...
    public static void fixInputMethodManagerLeak(Context destContext) {
        ...
        for (String view:viewArray) {
            try{
                failed = inputMethodManager.getClass().getDeclaredField(view);
                if (!failed.isAccessible()) {
                    failed.setAccessible(true);
                }
                failedObject = failed.get(inputMethodManager);
                if (failedObject != null && failedObject instanceof View) {
                    View fileView = (View) failedObject;
                    if (fileView.getContext() == destContext) { // 被InputMethodManager持有引用的context是想要目标销毁的
                        failed.set(inputMethodManager, null); // 置空，破坏掉path to gc节点
                    } else {
